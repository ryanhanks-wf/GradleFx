...
import org.gradlefx.util.TestFile
...
class PathFactoryTest extends Specification {
    ...
    def createsPathForAJarFile() {
        ...
        expect:
        def path = factory.path(tmpDir.file('a.swc'))
        path.url == 'jar://$ROOT_DIR$/a.swc!/'
        path.relPath == '$ROOT_DIR$/a.swc'
    }
    ...
    def filePathsAreEqualWhenTheyPointToTheSameEntryInTheSameFile() {
        TestFile subDir = tmpDir.file('sub')
        TestFile childFile = tmpDir.file('sub/a/b.swc')
        ...
        // Using replacement variables
        factory.path(childFile) == factory.path('jar://$SUB_DIR$/a/b.swc!/')
        factory.path(childFile) == factory.path('jar://$SUB_DIR$/c/../a/b.swc!/')
