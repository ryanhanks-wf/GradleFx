    ...
    void unzipTo(File target, boolean nativeTools) {
        ...
        file.withInputStream {InputStream instr ->
            ...
            while (entry = zipStr.getNextEntry()) {
                if (entry.directory) {
                    Assert.assertTrue("Duplicate directory '$entry.name'", dirs.add(entry.name))
                }
                if (!entry.name.contains('/')) {
                    continue
                }
                def parent = StringUtils.substringBeforeLast(entry.name, '/') + '/'
                Assert.assertTrue("Missing dir '$parent'", dirs.contains(parent))
            }
        }
        ...
        if (nativeTools && isUnix()) {
            ...
            Assert.assertThat(process.waitFor(), org.hamcrest.Matchers.equalTo(0))
            return
        }
